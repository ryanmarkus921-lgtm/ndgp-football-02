...
function shortDate(v){return v ? new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric"}).format(new Date(v+"T12:00:00")):""}

function gameResult(g){
  if(g.status!=="completed" || g.our_score==null || g.opponent_score==null) return "upcoming";
  if(Number(g.our_score)>Number(g.opponent_score)) return "win";
  if(Number(g.our_score)<Number(g.opponent_score)) return "loss";
  return "tie";
}

function resultLabel(g){
  const r=gameResult(g);
  return r==="win"?"WIN":r==="loss"?"LOSS":r==="tie"?"TIE":g.status==="cancelled"?"CANCELLED":"UPCOMING";
}

function recordFor(gamesIn){
  let w=0,l=0,t=0;
  gamesIn.forEach(g=>{const r=gameResult(g); if(r==="win")w++; else if(r==="loss")l++; else if(r==="tie")t++;});
  return t ? `${w}-${l}-${t}` : `${w}-${l}`;
}
...
function renderDashboard(){
  const seasonGames=games.filter(g=>Number(g.season)===2026);
  ...
  $("dashboard").insertAdjacentHTML("afterbegin", `
    <section class="stats-grid" id="admin-stats">
      ...
      <article class="stat-card">
        ...
        <strong class="stat-text">${next ? esc(next.opponent) : "None"}</strong>
        <small>${next ? esc(shortDate(next.game_date))+" • "+esc(next.game_time||"") : "No upcoming game"}</small>
      </article>
      <article class="stat-card">
...
function renderGames(){
  ...
  $("games-admin").innerHTML=`
    <div class="toolbar">
      <div><p class="eyebrow">Schedule Management</p><h2>Games</h2></div>
    ...
    <div class="season-dropdowns">
      ${years.map(year=>`
        <details class="season-dropdown" ${year===2026?"open":""}>
          <summary><span>${year===2026?"2026 SEASON":`${year} HISTORY`}</span><span class="season-count">${games.filter(g=>Number(g.season)===year).length} games</span></summary>
...
function renderAdminSeasonGames(year){
  const seasonGames=games.filter(g=>Number(g.season)===year).sort((a,b)=>a.game_date.localeCompare(b.game_date));
  if(!seasonGames.length) return `<div class="empty-admin">No games have been added for ${year}.</div>`;
  return seasonGames.map(g=>`
    <div class="admin-row">
      <div>
        <div class="row-title">${g.week?`Week ${esc(g.week)} • `:""}${esc(g.opponent)}</div>
        <div class="row-sub">${dateDisplay(g.game_date)}${g.game_time?" • "+esc(g.game_time):""}${g.location?" • "+esc(g.location):""}</div>
        <div class="row-sub"><span class="status-pill ${gameResult(g)}">${esc(resultLabel(g))}</span>${g.status==="completed"&&g.our_score!=null?` <b>${g.our_score}–${g.opponent_score}</b>`:""}</div>
      </div>
      <div class="row-actions">
        ${g.status!=="completed"?`<button data-quick-game="${g.id}">SCORE</button>`:`<button data-quick-game="${g.id}">SCORE</button>`}
...
function showQuickScore(game){
  $("game-form-wrap").innerHTML=`
    <form id="quick-score-form" class="panel quick-score-panel">
      <div class="quick-score-head">
        <div><p class="eyebrow">Game Day</p><h3>Notre Dame vs ${esc(game.opponent)}</h3><p class="muted">${dateDisplay(game.game_date)}${game.game_time?" • "+esc(game.game_time):""}</p></div>
        <button type="button" class="secondary" id="close-score">CLOSE</button>
      </div>
      <div class="score-inputs">
        <label>Notre Dame<input name="our_score" type="number" min="0" inputmode="numeric" value="${game.our_score??""}" required></label>
        <div class="score-dash">–</div>
        <label>${esc(game.opponent)}<input name="opponent_score" type="number" min="0" inputmode="numeric" value="${game.opponent_score??""}" required></label>
      </div>
      <div class="quick-actions">
        <button type="submit">SAVE FINAL</button>
        <button type="button" class="secondary" id="save-score">SAVE SCORE ONLY</button>
      </div>
      <div id="quick-score-msg" class="muted"></div>
    </form>
  `;
  $("close-score").onclick=()=>{$("game-form-wrap").innerHTML=""};
  $("quick-score-form").onsubmit=async e=>{
    e.preventDefault();
    await updateScore(game.id,new FormData(e.target),"completed");
  };
  $("save-score").onclick=async()=>{
    await updateScore(game.id,new FormData($("quick-score-form")),"upcoming");
  };
}

async function updateScore(id,form,status){
  const our=Number(form.get("our_score")), opp=Number(form.get("opponent_score"));
  if(!Number.isFinite(our)||!Number.isFinite(opp)||our<0||opp<0) return alert("Enter valid scores.");
  const {error}=await db.from("games").update({our_score:our,opponent_score:opp,status}).eq("id",id);
...
function showGameForm(game=null){
  $("game-form-wrap").innerHTML=`
    <form id="game-form" class="panel form-panel">
      <div class="toolbar"><div><p class="eyebrow">${game?"Edit Game":"New Game"}</p><h3>${game?"Update Game":"Add Game"}</h3></div><button type="button" class="secondary" id="cancel-game">CANCEL</button></div>
      <div class="form-grid">
        ...
        <label>Home/Away<select name="home_away">${["Home","Away","Neutral"].map(x=>`<option ${game?.home_away===x?"selected":""}>${x}</option>`).join("")}</select></label>
        ...
        <label>Notre Dame Score<input name="our_score" type="number" min="0" value="${game?.our_score??""}"></label>
        <label>Opponent Score<input name="opponent_score" type="number" min="0" value="${game?.opponent_score??""}"></label>
        <label class="full">Notes<input name="notes" value="${esc(game?.notes||"")}" /></label>
      </div>
      <button style="margin-top:14px">${game?"SAVE CHANGES":"CREATE GAME"}</button>
    </form>
  `;
  ...
  $("game-form").onsubmit=async e=>{
    ...
    const res=game?await db.from("games").update(data).eq("id",game.id):await db.from("games").insert(data);
...
function renderRoster(){
  $("roster-admin").innerHTML=`
    <div class="toolbar"><div><p class="eyebrow">Player Management</p><h2>Roster</h2></div><button id="new-player">ADD PLAYER</button></div>
    <div class="roster-tools"><input id="roster-search" placeholder="Search by name or number"></div>
    <div id="player-form-wrap"></div>
    <div id="roster-list" class="admin-list"></div>
  `;
  $("new-player").onclick=()=>showPlayerForm();
  $("roster-search").oninput=e=>drawRosterList(e.target.value);
  drawRosterList("");
}

function drawRosterList(query){
  const q=query.trim().toLowerCase();
  const players=roster.filter(p=>`${p.name} ${p.jersey_number} ${p.position} ${p.grade}`.toLowerCase().includes(q));
  $("roster-list").innerHTML=players.length?players.map(p=>`
    <div class="admin-row">
      <div><div class="row-title">#${esc(p.jersey_number)} • ${esc(p.name)}</div><div class="row-sub">${esc(p.position)} • ${esc(p.grade)}${p.active===false?" • INACTIVE":""}</div></div>
      <div class="row-actions"><button data-edit-player="${p.id}">EDIT</button><button class="danger" data-delete-player="${p.id}">DELETE</button></div>
    </div>
  `).join(""):`<div class="empty-admin">No players match your search.</div>`;
...
function showPlayerForm(player=null){
  $("player-form-wrap").innerHTML=`
    <form id="player-form" class="panel form-panel">
      <div class="toolbar"><div><p class="eyebrow">${player?"Edit Player":"New Player"}</p><h3>${player?"Update Player":"Add Player"}</h3></div><button type="button" class="secondary" id="cancel-player">CANCEL</button></div>
      <div class="form-grid">
        <label>Name<input name="name" value="${esc(player?.name||"")}" required></label>
        <label>Jersey Number<input name="jersey_number" type="number" min="0" value="${player?.jersey_number??""}" required></label>
      ...
      <button style="margin-top:14px">${player?"SAVE CHANGES":"CREATE PLAYER"}</button>
    </form>
  `;
...
function renderSettings(settings){
  $("settings-admin").innerHTML=`
    <div class="toolbar"><div><p class="eyebrow">App Configuration</p><h2>Settings</h2></div></div>
